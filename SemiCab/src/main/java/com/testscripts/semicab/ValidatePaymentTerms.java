...
public class ValidatePaymentTerms extends Base{
	@org.testng.annotations.Test
	public void validatePaymentTermsforContractLoad() throws Throwable {
		...
		test=report.startTest("ValidatePaymentTerms");
		...
		Assert.assertEquals(LoadNum1.equals(LoadNum), true);
		...
		Assert.assertEquals(l.getLoadEllispsis().isDisplayed(), true);
		...
		Assert.assertEquals(true,i.getInvoiceCustomer().getText().equalsIgnoreCase(fl.getDataFromExel(exel, 23, 1, "LoadData")));
		...
		String InvoicePT = i.getInvoicePaymentTerms().getText();
		i.getInvoiceContractNum().click();
		cu.SwithToNewWindow(driver);
		cu.waitTillElementToBeVisible(driver, i.getContractPaymentTerms());
		Assert.assertEquals(true,InvoicePT.equalsIgnoreCase(i.getContractPaymentTerms().getText()));
		test.log(LogStatus.PASS,"Invoice Payment Terms==> " +InvoicePT+" Contract Payment Terms==> "+i.getContractPaymentTerms().getText());
